...
params.snpvcf = "WI.20220216.hard-filter.isotype.snpsonly.vcf.gz" // VCF containing all SNPs (only) for all strains of interest vs. reference genome of interest
params.indelvcf = "WI.20220216.hard-filter.isotype.indelsonly.vcf.gz" // VCF containing all INDELs (only) for all strains of interest vs. reference genome of interest
params.reffasta = "c_elegans.PRJNA13758.WS283.genomic.fa" // FASTA reference genome - for strain-specific genome creation; what variants were called against. Should be unzipped
params.refgtf = "c_elegans.PRJNA13758.WS283.canonical_geneset.gtf  " // GTF file for reference genome.  Transcripts must NOT have underscores in their name if salmon is to be run through EMASE. Modify transcript names to exclude them if necessary.
...
params.procgtfscriptdir = 'https://github.com/averydavisbell/wormstrainrnaiexpr/rnaseq/ase/processdiptranscriptomes' // Directory containing Python GTF worker scripts gtfnonchroverlaps.py and gtftoemasegenemapping.py
